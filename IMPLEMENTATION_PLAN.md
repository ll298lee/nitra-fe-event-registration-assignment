...
- [ ] `feat(validation)` wire `useValidation` (touched-field "reward early, punish late", D7,
      AC-V-5) + submit gate over `validateAll` + `FormField` errors + double-submit guard (AC-4.8).
...
| #TBD                                                                              | `feat/step-4-review-ui` — Step 4 PR 2: Review UI — single-column grouped summary (Attendee/Sessions/Add-ons) + itemized `PricingSummaryCard` (line items, VIP discount, Grand Total, no aggregate subtotal) + per-section Edit → jump-to-step; `formatDateTime` helper; shell `<h1>` sr-only for all steps (D35; AC-4.1/4.2/4.3) | _pending_ | **Open — awaiting human review** |
