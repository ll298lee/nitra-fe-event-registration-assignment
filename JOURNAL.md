# JOURNAL — the development journey, one commit at a time

> **Renamed from `PLAN.md` (2026-07-11).** This file is the per-commit journal — one `##`
> section per commit, in commit order (`CLAUDE.md` §5). The submission narrative now lives
> in `PLAN.md` (README §Submission). Historical entries below keep their original wording,
> including references to this file by its old name.
...
**Current state:** feature-complete. The setup/tooling work, the full 4-step
wizard (Step 1 Attendee Info → Step 2 Session Selection → Step 3 Add-ons →
Step 4 Review & Submit, plus the success screen), and the Phase-4 polish (UX,
responsive breakpoints, i18n en + zh-TW) are all merged — 25 PRs, 201 tests
green.
...
## docs: split the journal (JOURNAL.md) from the submission PLAN.md

The old `PLAN.md` had accumulated two jobs: the per-commit journal mandated by
`CLAUDE.md` §5 and the submission narrative mandated by `README.md` §Submission. Split
them: this journal is now `JOURNAL.md` (a git rename — history and entries preserved), and
a new `PLAN.md` holds the submission narrative (planning approach, key decisions,
dependency rationale, AI usage, challenges, future improvements), synthesized from this
journal, `IMPLEMENTATION_PLAN.md`, and the conversations of all 25 merged PRs.

- **Living references repointed to `JOURNAL.md`:** `CLAUDE.md` §1.5/§2.6/§5, the PR
  template's journal checkbox, `IMPLEMENTATION_PLAN.md`'s header note + D43, and the
  Vitest example-test comment.
- **Historical `##` entries above are untouched** — headings mirror commit subjects and
  bodies document what each commit contained, old file name included. `README.md` stays
