...
| #TBD                                                                              | `feat/step-4-validation-logic` — Step 4 PR 1: pure submit-time validation logic (`validation.js` predicates + `validateAll` per-step error map) + tests; ticket-required + session/workshop-conflict attribution spec-gap fills (D34; AC-V-1..4/6, AC-4.4–4.6, AC-1.6/1.7) | _pending_ | **Open — awaiting human review** |
