...
>
> **Not** the submission `PLAN.md`, nor the per-commit `JOURNAL.md`. `PLAN.md` (README
> §Submission) is the submission narrative; `JOURNAL.md` (renamed from the original `PLAN.md`
> journal) is the per-commit development log; this file is the engineering plan. Do not
> conflate the three.
...
| `src/i18n/{index,en,zh-TW}.js` + boot | `vue-i18n` instance + `en` (default) & `zh-TW` locales; all app-chrome copy extracted behind it, header `LanguageSwitcher` (D14/D45).                                                                          |
...
| **D43** | **Phase-4 UX polish — spec-gap fills across several UX themes (no Figma frames), one branch / several atomic commits.** _Scope expanded by the user mid-PR_ from the original three to four themes — **loading/disabled states, transitions & animations, stepper navigation feel, validation error experience**. None has a Figma frame, so each is a recorded spec-gap fill (§7), not a measured value. May **split into two PRs at review** (transitions+stepper / validation+disabled) if it exceeds a ~20-min review (§2.6). **(a) Stable scrollbar gutter (user-reported).** The document (`<html>`) is the scroll container (no Quasar `QLayout`; `App.vue` → `router-view` → `IndexPage`'s `min-h-screen` div), so a tall step gains a viewport scrollbar while a short step does not — the width difference shifts the centered layout horizontally when switching steps. Fix: `html { scrollbar-gutter: stable }` in `src/css/app.scss` (global, alongside the existing UA normalize). Chosen over `overflow-y: scroll` (which paints a **permanent empty track** on short steps); `stable` reserves the gutter without a visible track. On overlay-scrollbar platforms (macOS default) scrollbars take no space, so it is a harmless no-op there — which is correct, since those platforms never show the shift. **(b) Skeleton loaders replace the bare `q-spinner`.** All four steps gate their content on a `loading` ref off the async facade (D1, `SIMULATED_LATENCY_MS = 250`); the loading branch swaps the centered spinner for content-shaped `q-skeleton` placeholders (Quasar built-in, auto-registered) approximating each step's card layout, so first-paint shows structure instead of a spinner and there is no jump when data resolves. _(Steps 1–3 landed in the skeleton commit; Step 4/Review was added in the disabled-states commit — see JOURNAL.md.)_ **(c) Step fade transition.** The `v-if`/`v-else-if` step swap in `IndexPage`'s `<main>` is wrapped in `<Transition name="step" mode="out-in">` with a 150ms opacity + 4px `translateY` fade + a `prefers-reduced-motion` guard, replacing the hard cut. Classes live in a **non-scoped** `<style>` (Vue applies the `step-*` classes to the child step's root, which carries the _child's_ scope attribute — scoped rules here would not match; names are unique to avoid clashes); timing/opacity only — no tokens/hex, §1.4 unaffected. **(d) Stepper navigation feel, (e) validation error experience, (f) disabled-state polish** — the user-expansion themes; each documented in its own `JOURNAL.md` journal entry (§5) as it lands. **Commits:** (a) `fix(ux)` scrollbar, (b) `feat(ux)` skeletons, (c) `feat(ux)` step transition, then (d)/(e)/(f) as separate `feat(ux)` commits — each carries its own `JOURNAL.md` entry. | Frames are the visual source of truth (§1.3) but none exists for a scrollbar gutter, a loading skeleton, or a step transition — so all three are designed and recorded as spec-gap fills rather than invented silently (§2 anti-pattern). `scrollbar-gutter: stable` is the idiomatic remedy for the exact shift the user reported and is safe on every platform (reserves the gutter where classic scrollbars exist — Windows/Linux, macOS "always show" — and no-ops on overlay). **Verification caveat (recorded):** headless Chromium uses **overlay** scrollbars (0-width), so the visual shift **cannot be reproduced in `agent-browser`**; verification confirms (i) `getComputedStyle(document.documentElement).scrollbarGutter === 'stable'` (the rule is applied to the real scroll container) and (ii) no regression across all four steps — the shift-on-classic-scrollbars is reasoned from the property's spec, not screenshotted. Skeletons/transition are verified visually + by component test (loading branch renders skeletons, resolved branch renders content). |

| **D44** | **Phase-4 responsive — adapt at `tablet 768` / `desktop 1024` (spec-gap fill; the Figma is a fixed 1440 desktop frame with no responsive frames, §4).** Mobile-first: the existing desktop values become the `desktop:`/`tablet:` overrides over reduced base (mobile) values, using **only** the `tablet`/`desktop` breakpoint tokens from `src/unocss/index.js` (§4), never arbitrary widths. **(a) Band insets scale** — header `px-4 tablet:px-8 desktop:px-12` (mobile 16 → tablet 32 → desktop 48); stepper/main/footer `px-4 tablet:px-8 desktop:px-30` (16 → 32 → 120). The header keeps its smaller desktop inset (48 vs 120) per the shell frames. **(b) Step 1** ticket cards `grid-cols-1 tablet:grid-cols-3` (stack → 3-up); the two-up name/email + phone/company rows `flex-col gap-5 tablet:flex-row tablet:gap-6` (stack → side-by-side); Job Title / Shipping stay full-width. **(c) Step 2** session grid `grid-cols-1 tablet:grid-cols-2`. **(d) Step 3** the list + Order-Summary sidebar is `flex-col … desktop:flex-row` and the aside `w-full desktop:w-[380px] desktop:shrink-0` — **two-column only from desktop (1024)**, stacked (summary **below** the list) at tablet/mobile, because a 380px sidebar + gap over the tablet content width (≤704) leaves the add-on cards too narrow. **(e) Stepper** collapses to a compact numbered rail below tablet: **only the current step's label** shows (`inline`), non-current labels are `hidden tablet:!inline`; connector margin `mx-2 tablet:mx-4`. Step 4 (Review), the success screen, and all cards are already single-column/fluid — no change. **Desktop parity preserved:** at ≥1024 every value equals the pre-change design (header 48 / content 120, `grid-cols-3`, `grid-cols-2`, two-column, all labels), so the 1440 verify-gate is byte-identical — confirmed in-browser via `getComputedStyle` @375/768/1024/1440. **Trap hit + fixed (recorded gotcha):** the stepper label reveal first used `tablet:inline`, which rendered nothing at ≥768 because **Quasar ships `.hidden { display:none !important }`** (its visibility helper) that out-ranks UnoCSS's non-important `.tablet\:inline`; the fix is the **`!` important modifier** `tablet:!inline` (the app already uses this pattern in `IndexPage`'s `disabled:!opacity-50`). The grid/flex/padding responsive utilities were unaffected — Quasar has no competing `!important` rule for those properties. **(f) Review-driven refinements (adversarial review pass, 2026-07-11), both fixed + tested:** **(i)** the segmented tab controls (Step-2 day tabs, Step-3 category tabs) got no responsive treatment — at ≤375 the widest label ("Meal Packages") wrapped out of its fixed `h-8` pill and the `w-fit` strip pushed the whole page into horizontal scroll (clipping "Merchandise" + stepper "4"). Hardened so the strip **wraps to a second row** on mobile (`flex-wrap max-w-full`; `tablet:flex-nowrap` restores the single-line `w-fit` strip from 768) with each pill kept single-line via `whitespace-nowrap` (+ `shrink-0`, `px-4 tablet:px-5`) — so no label overflows its `h-8` pill and the page never gains horizontal scroll (verified at 375/360/320: `pageOverflow=false`, pill height stays 32; a first attempt with `overflow-x-auto` failed because a `nowrap` flex row's min-content equals its full width, so `max-w-full` couldn't shrink the flex-item chain to trigger the scroll). Step-2 day tabs fixed preventively — their short "Nov 15" labels don't overflow yet but share the pattern. **(ii)** the stepper's mobile label collapse uses `display:none`, which removes non-current labels from the **accessibility tree** — non-current steps announced as a bare digit, completed steps as an unnamed button (their check glyph is `aria-hidden`). Added `:aria-label="step.label"` to each step button so its accessible name is stable at every width (also normalizes the desktop name "1 Attendee Info" → "Attendee Info", consistent across completed/current/upcoming). | Figma is the visual source of truth (§1.3) but ships no responsive frames (fixed 1440), so the breakpoint behavior is a recorded spec-gap fill (§1.2/§7), not a measured value — mobile-first with the two mandated breakpoint tokens keeps the desktop frame pixel-identical while the layout degrades gracefully below it. Two-column-from-desktop (not tablet) is a deliberate width judgment; showing only the active stepper label on mobile keeps the wizard's "where am I" context without overflowing four labels + connectors into ~340px. **Known trade-off (flagged for review):** with only two breakpoints and an absolute 120px desktop inset, content is at its tightest right at 1024 (≈784px, the two-column Step-3 cards ≈372px) and widens toward 1440; a max-width-centered container would smooth this but is a larger structural refactor (full-bleed dividers need inner wrappers) and was deferred to keep the diff focused and 1440-parity provably identical. |

| **D45** | **i18n extraction — inject-a-translator, two locales (`en` + `zh-TW`), header switcher (implements D14, extends its scope).** Extract **all app-chrome copy** behind `vue-i18n@11` (Composition mode, `legacy:false`, `globalInjection`), registered via `src/boot/i18n.js`; messages in `src/i18n/{en,zh-TW}.js` (mirrored key-for-key, 105 keys each). **(a) Validation stays framework-free via an injected `t` (user-chosen over returning error codes).** `validation.js` predicates/`validateAll`/`summarizeErrors` take a `t(key, params)` param and return translated strings; `useValidation` grabs `useI18n({useScope:'global'}).t` and threads it into `createValidation`; the pure-logic tests pass the real `i18n.global.t`, so their assertions read against the shipped `en` copy unchanged. This keeps the pure boundary (no `vue-i18n` import in `logic/`) while moving every string into the locale files. **(b) Scope grown to `zh-TW` (Traditional Chinese) on user request** — beyond D14's `en`-only. i18n is a D-decision (never in `README.md`), so this delta is recorded **here**, not via a `SPEC_ADDENDUM.md` (that procedure is for README/functional-spec growth, §2). **(c) Header `LanguageSwitcher` (EN \| 中文) — a deliberate Figma spec-gap fill** (§7): the frame is English-only with no switcher, so a tokenized segmented control (reusing the Step-2/3 tab treatment) is added top-right; **`en` stays the default locale, so the shipped English UI is byte-identical to the frame** (parity preserved) and `zh-TW` is reachable by toggle. **(d) Translation boundary = app chrome only.** Only the app's own strings (headings, labels, placeholders, buttons, banners, validation + conflict messages) are keyed. **Data-layer content is NOT translated** — event/ticket/session/add-on **names, descriptions, perks, and prices come from `src/mocks/*`, which the user directed must not be touched**; it is data, not UI copy, so it renders identically in both locales (a `zh-TW` user sees Chinese chrome around English data — the correct i18n boundary). **Date and currency formatting are also out of scope** (`utils/datetime.js` wall-clock formatter + `utils/pricing.js` `formatCurrency`): they are locale-independent formatters, not copy, and re-localizing them (Chinese month names, `US$`) would enlarge the diff and risk `datetime`'s deterministic Figma-parity (D4) — both locales render identical dates (`Nov 15`) and prices (`$299`). Flagged for review; a follow-up can add `vue-i18n` number/date formats if desired. **(e) Conflict copy consolidated (resolves the D14/D34d known-consequence):** `WorkshopCard`'s in-card status and `validation.js`'s submit-error now both draw from a shared `conflict.*` namespace, so the two surfaces cannot drift. **(f) `STEPS` carries only `key`; labels moved to `steps.<key>`** — `IndexPage` builds a `localizedSteps` computed so the stepper + sr-only landmark re-render on locale switch. **(g) Template statics use `$t`; script-level strings use `useI18n({useScope:'global'})`; `SessionCard`/`WorkshopCard` moved their spots/conflict computeds into the template `$t`. Tests get the i18n plugin globally in `setup-file.js`** so mounted components resolve real `en` copy and every existing copy assertion stays green. | The user chose **inject-a-translator** over the return-error-codes alternative (smaller, lower-risk diff: `validation.js` still returns strings, downstream components + most logic-test assertions unchanged) while still moving all copy behind i18n and keeping `logic/` framework-free. Two locales + a switcher demonstrate the wiring end-to-end. The chrome-only boundary follows directly from the user's "don't touch `/mocks`" constraint and the nature of the content (data vs copy). **Verified:** `yarn check` + `yarn test:unit:ci` green (201 tests); `en`/`zh-TW` key + placeholder parity checked programmatically (105/105, only the intentional plural-count difference on `step2.counter`); in-browser `agent-browser` @1440 — `en` byte-identical to the frame + switcher, `zh-TW` fully translated across Step 1 and the Step-4 validation-error state (banner, `— （必填）` markers, red section cards, stepper error), mock data correctly left in English. |

**New dependency introduced:** `vue-i18n@11.4.6` (per D14/D45) — the only planned addition; installed. Registered via
`src/boot/i18n.js`; Vite/Vitest define the `__VUE_I18N_*` feature flags. Locales: `en` (default,
byte-identical to the frame) + `zh-TW` (D45).
...
| C-5 | conflict **message** copy — now shared `conflict.*` i18n keys (`WorkshopCard` + `validation.js`, D14/D45); both locales verified in-browser | SFC  |
...
### 5.12 Cross-cutting — Responsive (Phase 4, D44)

Verified primarily via **VIS** (`agent-browser` computed styles @ 375/768/1024/1440 — jsdom applies
no media queries), with **SFC** class-presence guards so the responsive wiring can't be silently
dropped in a refactor.

- **AC-R-1** Given Step 1, When the viewport is below `tablet` (768), Then the ticket cards stack to
  one column and the two-up form rows stack; from 768 up they are 3-up / side-by-side.
- **AC-R-2** Given Step 2, When below `tablet`, Then the session grid is one column; from 768 up,
  two columns.
- **AC-R-3** Given the stepper, When below `tablet`, Then only the current step's label is shown
  (compact numbered rail); from 768 up, all labels show.
- **AC-R-4** Given Step 3, When below `desktop` (1024), Then the add-on list and Order-Summary
  sidebar stack (summary full-width, below the list); from 1024 up, they are a two-column row with a
  380px sidebar.
- **AC-R-5** Given any step at `desktop` width (1440, the Figma frame), When rendered, Then the
  layout is byte-identical to the pre-responsive design (header 48 / content 120 insets, `grid-cols-3`,
  `grid-cols-2`, two-column Step 3, all stepper labels).

| AC  | Test                                                                                                | Kind      |
| --- | --------------------------------------------------------------------------------------------------- | --------- |
| R-1 | `StepAttendee.spec.js` → "responsive stack→grid classes for tickets and form rows" + VIS @375/768   | SFC + VIS |
| R-2 | `StepSessions.spec.js` → "responsive single→two column classes on the session grid" + VIS @375/768  | SFC + VIS |
| R-3 | `WizardStepper.spec.js` → "only current step label below tablet, all from 768px up" + VIS @375/1440 | SFC + VIS |
| R-4 | `StepAddons.spec.js` → "stacks list + summary below desktop, two-column from 1024px up" + VIS       | SFC + VIS |
| R-5 | VIS — `getComputedStyle` @1440 equals the pre-change insets/grids/labels (parity)                   | VIS       |

### 5.13 Cross-cutting — Internationalization (Phase 4, D45)

Verified by the **whole existing suite** (component + logic tests now mount/run with `vue-i18n`
installed and assert the real `en` copy — a regression guard that every rendered string still
resolves), a **key-parity PF** check, and **VIS** in both locales.

- **AC-I18N-1** Given the app in the default locale, When any step renders, Then every UI-chrome
  string comes from the `en` locale and the output is byte-identical to the pre-i18n / Figma English.
- **AC-I18N-2** Given the header `EN | 中文` switcher, When 中文 is clicked, Then the whole wizard
  re-renders in `zh-TW` (chrome only) and back to `en` when EN is clicked; `en` is the default.
- **AC-I18N-3** Given `src/mocks/*` data (event/ticket/session/add-on names, descriptions, perks,
  prices) and date/currency formatting, When the locale switches, Then they render **identically** in
  both locales (data + formatters are not translated, D45).
- **AC-I18N-4** Given `validation.js`, When it builds a message, Then it uses the injected `t` and
  imports nothing from `vue`/`vue-i18n` (framework-free boundary preserved).
- **AC-I18N-5** Given `en.js` and `zh-TW.js`, When compared, Then they have identical key sets and
  matching interpolation placeholders (bar the intentional plural difference on `step2.counter`).
- **AC-I18N-6** Given a time conflict, When surfaced on the Step-3 card and at Step-4 submit, Then
  both draw from the shared `conflict.*` keys (cannot drift, D14).

| AC     | Test                                                                                            | Kind      |
| ------ | ----------------------------------------------------------------------------------------------- | --------- |
| I18N-1 | Full component suite (mounts with i18n, asserts `en` copy) + VIS @1440 `en` vs frame            | SFC + VIS |
| I18N-2 | VIS — `agent-browser` toggle `en`↔`zh-TW`, whole wizard re-renders                              | VIS       |
| I18N-3 | VIS — `zh-TW` render shows English mock data + `$`/`Nov 15` formats unchanged                   | VIS       |
| I18N-4 | `validation.test.js` passes the real `t`; static check — no `vue`/`vue-i18n` import in `logic/` | PF        |
| I18N-5 | Node key/placeholder-parity check (105/105, `en` vs `zh-TW`)                                    | PF        |
| I18N-6 | `conflict.*` keys shared by `WorkshopCard` + `validation.js`; VIS both surfaces in `zh-TW`      | SFC + VIS |
...
- [x] `feat(responsive)` adapt at `tablet 768` / `desktop 1024` (D44). Mobile-first: band insets
      (`px-4 tablet:px-8 desktop:px-12|px-30`), Step-1 tickets/form + Step-2 sessions stack→grid at
      768, Step-3 two-column from 1024, compact stepper (current label only) below 768. Desktop 1440
      byte-identical (parity); AC-R-1..5. **PR (this change).**
- [x] `feat(i18n)` extract UI copy behind `vue-i18n` (D14/D45). Inject-a-translator (`validation.js`
      stays framework-free); two locales `en` (default, frame-parity) + `zh-TW`; header `EN | 中文`
      switcher (spec-gap fill); conflict copy consolidated under `conflict.*`; chrome-only boundary —
      `src/mocks/*` data + date/currency formatters untranslated (D45). 201 tests green; both locales
      verified in-browser @1440.
...
| **Language switcher** (Figma is English-only, no switcher drawn)                       | _no frame_ — Phase-4 `feat(i18n)`                        | **Spec-gap fill (D45):** a tokenized `EN \| 中文` segmented control in the header top-right (reusing the Step-2/3 tab treatment), added so the `zh-TW` locale is reachable. `en` stays the default locale, so the shipped English UI is byte-identical to the frame — the switcher is the only header addition. Verified in-browser at 1440 in both locales.                                                                                                                                                                                      |
| **Responsive breakpoint behavior** (Figma is a fixed 1440 frame, no responsive frames) | _no frame_ — Phase-4 `feat(responsive)`                  | **Spec-gap fill (D44):** mobile-first adapt at `tablet 768` / `desktop 1024` (the two breakpoint tokens, §4) — band insets scale, Step-1/2 grids + form rows stack→grid at 768, Step-3 two-column from 1024 (summary below when stacked), compact stepper (current label only) below 768. Desktop 1440 byte-identical to the frame (parity). Trap: Quasar's `.hidden{display:none!important}` forced `tablet:!inline` for the label reveal. Trade-off: tightest content at ~1024 (absolute 120px inset); max-width container deferred. AC-R-1..5. |
...
| [#22](https://github.com/ll298lee/nitra-fe-event-registration-assignment/pull/22) | `docs/plan-sync-phase4-review-log` — plan hygiene (no code): reconcile the Phase-4 checklist + add the missing PR #20/#21 review-log rows                                                                                                                                                                                                                                                                                                                                                                                                       | ll298lee | **Merged** (2026-07-10) |
| [#23](https://github.com/ll298lee/nitra-fe-event-registration-assignment/pull/23) | `feat/responsive-breakpoints` — Phase-4 responsive (D44): mobile-first adapt at `tablet 768` / `desktop 1024` — band insets scale, Step-1/2 grids + form rows stack→grid at 768, Step-3 two-column from 1024, compact stepper below 768; 1440 byte-identical (AC-R-1..5)                                                                                                                                                                                                                                                                        | ll298lee | **Merged** (2026-07-10) |
| [#24](https://github.com/ll298lee/nitra-fe-event-registration-assignment/pull/24) | `feat/i18n-copy-extraction` — Phase-4 i18n (D14/D45): extract all app-chrome copy behind `vue-i18n` (`en` default + `zh-TW`), inject-a-translator (validation.js stays framework-free), header `EN \| 中文` switcher (spec-gap fill), conflict copy consolidated under `conflict.*`; chrome-only boundary (mocks + date/currency untranslated) (AC-I18N-1..6)                                                                                                                                                                                   | ll298lee | **Merged** (2026-07-10) |
