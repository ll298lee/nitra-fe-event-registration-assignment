...
- **Figma frames:** Step 1 `1069:968` · Step 2 `1072:912` · Step 3 `1073:899` (Workshops) &
  ...
  (stepper error + error banner + disabled submit, D37). _No Meal-Packages-tab frame and no
  workshop-conflict frame exist — see D31 spec-gap fills._
...
- **AC-4.2** Given selections, When the breakdown renders, Then every line (its per-line subtotal) +
  the VIP workshop discount + a grand total in `$X,XXX.XX`. Per Figma `1074:897` there is **no
...
- [ ] `feat(review)` async `submitRegistration` + pending + double-submit guard + terminal success
      screen, "Back to Home" (D15, AC-4.7/4.8, AC-S-2/S-4, D36h) — **next PR**.
